...
const Card = ({ name, image, dataId, seasonNo }) => {
  return (
    <Link href={`/shows/season/${dataId}/${seasonNo + 1}`}>
      <div className="card group/card lg:w-[9.5rem] lg:h-[100%] md:w-[9.5rem] min-[360px]:w-[6rem] md:h-[100%] min-[360px]:h-[60%] rounded-lg mr-6 relative transition-all duration-500 z-10 hover:z-20 hover:cursor-pointer mb-4">
        <Image
          src={
            image
        ...
        <div className="absolute h-10 rounded-md flex items-center episode-card-gradient w-[100%] bottom-0 left-0 text-white">
          <p className="flex items-center ml-3 text-sm font-bold">{name}</p>
        </div>
      </div>
    </Link>
  );
