...
export const getDocumentaryMovies = async () => {
  const responseHi = await fetch(
    movieURL("release_date.desc", "99", "", "hi", "122")
  );
  const responseEn = await fetch(
    movieURL("release_date.desc", "99", "", "en", "122")
  );
...
export const getDocumentaryShows = async () => {
  const responseHi = await fetch(
    tvURL("release_date.desc", "99", "", "hi", "122")
  );
  const responseEn = await fetch(
    tvURL("release_date.desc", "99", "", "en", "122")
  );
